...
class PurchaseController extends Controller
{
    ...
    public function store(PurchaseRequest $request) : JsonResponse  {
        ...
        $totalDiscount = $request->total_discount ?? 0;  // Default to 0 if not provided
        $totalDiscountType = $request->total_discount_type ?? 'flat';  // Default to 'flat' if not provided
        $totalDiscountAmount = $totalDiscount;
            ...
            if($request->operation == 'save' || $request->operation == 'convert'){
                ...
                $fillableColumns = [
                    ...
                    'total_discount' => $totalDiscountAmount,  // Save total discount
                     'total_discount_type' => $totalDiscountType,  // Save discount type (flat or percent)
