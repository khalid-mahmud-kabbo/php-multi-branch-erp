...
function normalizeDiscountFieldValue() {
    const discountTotalField = document.querySelector('[name="total_discount"]');
    if (!discountTotalField) return;

    let currentValue = (discountTotalField.value || '').trim();

    if (currentValue.startsWith('0') && currentValue.length > 1 && !currentValue.startsWith('0.')) {
        currentValue = currentValue.replace(/^0+/, '');
        discountTotalField.value = currentValue || '0';
    }
    ...
    if (
        ...
        discountTotalField.value = '0';
    }
}

function getInvoiceLevelDiscountAmount() {
    const discountTotalField = document.querySelector('[name="total_discount"]');
    const discountTypeField = document.querySelector('[name="total_discount_type"]');
    const applyDiscountCheckbox = document.querySelector('#apply_discount_checkbox');

    if (!applyDiscountCheckbox || !applyDiscountCheckbox.checked) {
        return 0;
    }

    const enteredDiscountValue = parseFloat(discountTotalField?.value) || 0;
    const discountType = discountTypeField?.value || 'flat';
    const sumOfTotalColumn = returnSumOfTotalColumn();

    if (discountType === 'percent') {
        return (sumOfTotalColumn * enteredDiscountValue) / 100;
    }

    return enteredDiscountValue;
}

function setBottomInvoiceTotal() {
    let sumOfTotalColumn = returnSumOfTotalColumn();
    const roundOff = parseFloat(getCurrentRoundOffAmount()) || 0;
    const shippingCharge = returnDecimalValueByName('shipping_charge');
    const invoiceDiscount = getInvoiceLevelDiscountAmount();

    const grandTotal = parseFloat(sumOfTotalColumn) + parseFloat(roundOff) + parseFloat(shippingCharge) - parseFloat(invoiceDiscount);

    $(".grand_total").val(_parseFix(grandTotal));

    // update converted amount
    convertToExchangeCurrencyAmount();

    // update payment amount for create page
    autoFillPaymentInputBox();
}

function calculateInvoiceTotals() {
    normalizeDiscountFieldValue();
    setBottomOfTableRecords();
}

// discount / invoice level change events
$(document).on(
    'input change',
    '[name="total_discount"], [name="total_discount_type"], #apply_discount_checkbox, [name="shipping_charge"], [name="round_off"]',
    function () {
